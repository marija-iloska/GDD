function [A_s] = gibbs_double_degree(iter_num, burn_in, thin_rate, A_init, lambda_init, C, mu, sig, gamma)
% Obtain dimensions
dim_y = length(C(:, 1));
% Initialize arrays to store samples
A_s = zeros(dim_y, dim_y, iter_num+1);
lambda_s = zeros(iter_num+1, dim_y, dim_y);
% Set the initial states of the Markov chains
A_s(:, :, 1) = A_init;
lambda_s(1,1,1) = lambda_init;
A_old = A_s(:, :, 1);
...
for i = 2:iter_num+1
    for j = 1:dim_y
        for k = 1:dim_y
            ...
            A_old(j,k) = 1;
            C_temp = C.*A_old; 
            log_pa1 = -lambda_s(j,k,i-1)*(sum(A_old(j,:) + A_old(:,k)')- A_old(j,k)) + logmvnpdf(C_temp(:)', mu', sig);
           
            % Conside that A(j, k)=0
            A_old(j,k) = 0;
            C_temp = C.*A_old; 
            log_pa0 = -lambda_s(j,k,i-1)*(sum(A_old(j,:) + A_old(:,k)')- A_old(j,k)) + logmvnpdf(C_temp(:)', mu', sig);
            ...
            lambda_s(j,k,i) = gamrnd(2, gamma + sum(A_old(j,:) + A_old(:,k)')- A_old(j,k));
        end
    end
    A_s(:, :, i) = A_old; 
